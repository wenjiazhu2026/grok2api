...
if [ -f "${GROK2API_CONFIG_SOURCE}" ]; then
    ...
    # QualityGuard defaults
    : "${GROK2API_QUALITY_GUARD_ENABLED:=false}"
    : "${GROK2API_QUALITY_GUARD_MODEL:=grok-3.5"
    : "${GROK2API_QUALITY_GUARD_MODE:=active}"
    : "${GROK2API_QUALITY_GUARD_ACTIVE_INTERVAL:=30m}"
    : "${GROK2API_QUALITY_GUARD_SOFT_TPS:=500}"
    : "${GROK2API_QUALITY_GUARD_HARD_TPS:=1000}"
    : "${GROK2API_QUALITY_GUARD_FAIL_CLOSED:=false}"
    : "${GROK2API_QUALITY_GUARD_ROTATION_URL:=}"
    : "${GROK2API_QUALITY_GUARD_ROTATION_TOKEN:=}"
...
qualityGuard:
  enabled: ${GROK2API_QUALITY_GUARD_ENABLED}
  model: "${GROK2API_QUALITY_GUARD_MODEL}"
  mode: "${GROK2API_QUALITY_GUARD_MODE}"
  activeInterval: ${GROK2API_QUALITY_GUARD_ACTIVE_INTERVAL}
  softTPS: ${GROK2API_QUALITY_GUARD_SOFT_TPS}
  hardTPS: ${GROK2API_QUALITY_GUARD_HARD_TPS}
  failClosed: ${GROK2API_QUALITY_GUARD_FAIL_CLOSED}
